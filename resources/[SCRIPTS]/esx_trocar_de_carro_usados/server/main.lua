	...
	if result[1] ~= nil then
		MySQL.Async.execute('UPDATE owned_vehicles SET owner = @target WHERE owner = @owner AND plate = @plate', {
			...
			if rowsChanged ~= 0 then
				TriggerClientEvent('esx_trocar_de_carro_usados:showAnim', _source)
				Wait(22000)
				TriggerClientEvent('esx_trocar_de_carro_usados:showAnim', _target)
				Wait(22000)
				...
				xPlayer.removeInventoryItem('contrato', 1)
			end
		end)
	else
...
ESX.RegisterUsableItem('contrato', function(source)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	TriggerClientEvent('esx_trocar_de_carro_usados:getVehicle', _source)
end)
