...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(1, Keys["F9"]) and PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
			...
			if Onjob then
				...
				if IsPedInAnyVehicle(playerPed,  false) and IsVehicleModel(GetVehiclePedIsIn(playerPed,  false), GetHashKey("comet2")) then
					StartNPCJob()
					Onjob = true
				else
