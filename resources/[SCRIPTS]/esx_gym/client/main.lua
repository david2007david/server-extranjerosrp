...
local rentbike = {
    {x = -1037.9,y = -2738.00,z = 20.16},
	{x = -1037.54,y = -273.703,z = 20.16}
}
            ...
            if dist <= 0.5 then
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						ESX.ShowNotification("Preparándote para el ~g~ejercicio...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Necesitas estirarte un momento antes de hacer otro ejericio, ~r~toma 60 segundos de estiramiento.~s~")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Necesitas una membresía para hacer ~r~ejercicio")
						end
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then
							...
							Citizen.Wait(30000)
							exports["gamz-skillsystem"]:UpdateSkill("Aguante", 0.5)
							exports["gamz-skillsystem"]:UpdateSkill("Fuerza", 0.5)
							ClearPedTasksImmediately(playerPed)
							ESX.ShowNotification("Necesitas estirarte un momento antes de hacer otro ejericio, ~r~toma 60 segundos de estiramiento.~s~")
    ...
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				...
				ShowFloatingHelpNotification("Presiona ~r~[E]~s~ para hacer algunas abdominales", vector3(situps[k].x, situps[k].y, situps[k].z+1.3))
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then	
							...
							Citizen.Wait(30000)
							exports["gamz-skillsystem"]:UpdateSkill("Capacidad Pulmonar", 0.2)
							exports["gamz-skillsystem"]:UpdateSkill("Fuerza", 0.2)
							exports["gamz-skillsystem"]:UpdateSkill("Aguante", 0.2)
							ClearPedTasksImmediately(playerPed)
							ESX.ShowNotification("Necesitas estirarte un momento antes de hacer otro ejericio, ~r~toma 60 segundos de estiramiento.~s~")
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("¡Bienvenido! Estamos abiertos las ~g~24 horas~s~ del día, Los ~g~7 Días~s~ de la semana.")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Tienda',
            elements = {
				{label = '&#x1F964 Batido de Proteínas ($50)', value = 'protein_shake'},
				{label = '&#x1F95B Agua ($10)', value = 'water'},
				{label = '&#x1F956 Barra de Proteínas ($100)', value = 'sportlunch'},
				{label = '&#x26A1 Gatorade ($200)', value = 'powerade'}
            }
        },
        function(data, menu)
