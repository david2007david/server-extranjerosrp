...
function OpenCloakroomMenu()
	...
	if grade == 'recruit' then
		table.insert(elements, {label = _U('taxi_wear'), value = 'recruit_wear'})
	elseif grade == 'officer' then
		table.insert(elements, {label = _U('taxi_wear'), value = 'officer_wear'})
	elseif grade == 'sergeant' then
		table.insert(elements, {label = _U('taxi_wear'), value = 'sergeant_wear'})
	elseif grade == 'intendent' then
		table.insert(elements, {label = _U('taxi_wear'), value = 'intendent_wear'})
	elseif grade == 'lieutenant' then
		table.insert(elements, {label = _U('taxi_wear'), value = 'lieutenant_wear'})
	elseif grade == 'chef' then
		table.insert(elements, {label = _U('taxi_wear'), value = 'chef_wear'})
	elseif grade == 'boss' then
		table.insert(elements, {label = _U('taxi_wear'), value = 'boss_wear'})
	end
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerEvent('esx_taxijob:updateBlip')
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			elseif type == 'helicopter' then
				shopCoords = Config.TaxiStations[station].Helicopters[partNum].InsideShop
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_taxijob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
						OpenBodySearchMenu(closestPlayer)
					elseif action == 'drive_bike' then
						TriggerServerEvent('esx_taxijob:message', GetPlayerServerId(closestPlayer), _U('being_drive_bike'))
						Opendrive_bikeadd(closestPlayer)
					elseif action == 'drive_truck' then
						TriggerServerEvent('esx_taxijob:message', GetPlayerServerId(closestPlayer), _U('being_drive_truck'))
						Opendrive_truckadd(closestPlayer)						
                    elseif action == 'drive' then
						TriggerServerEvent('esx_taxijob:message', GetPlayerServerId(closestPlayer), _U('being_drive'))
	...
	ESX.TriggerServerCallback('esx_taxijob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_biPorrulling:sendBill', GetPlayerServerId(player), 'society_taxi', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
ESX.ShowFloatingHelpNotification(CurrentActionMsg, vector3(playerX, playerY, playerZ))
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'taxi' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBosPorrusMenu', 'taxi', function(data, menu)
						menu.close()
