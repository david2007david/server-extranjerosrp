    ...
    while true do
        ...
        if IsControlJustReleased(1, Keys['K']) then
            ...
            if IsPedInAnyVehicle(GetPlayerPed(), false) then
                suelo = false
                notify("~r~No puedes usar esto cuando estas en un coche")
            end
        end
...
ESX.ShowFloatingHelpNotification(('Pulsa ~INPUT_PICKUP~ para ~y~levantarte'), vector3(playerX, playerY, playerZ))
