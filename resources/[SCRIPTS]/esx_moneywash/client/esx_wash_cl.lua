...
RegisterNUICallback('deposit', function(data, cb)
	TriggerServerEvent('esx_moneywash:depoPorrusit', data.amount)
	cb('ok')
end)

RegisterNUICallback('withdraw', function(data, cb)
	TriggerServerEvent('esx_moneywash:witPorruhdraw', data.amount)
	cb('ok')
end)
	...
	while true do		
		...
		for i=1, #Config.WASH, 1 do
			if(GetDistanceBetweenCoords(coords, Config.WASH[i].x, Config.WASH[i].y, Config.WASH[i].z, true) < Config.ZoneSize.x / 2) then
                ...
                ESX.ShowFloatingHelpNotification(_U('press_e_wash'), vector3(playerX, playerY, playerZ))
