local Keys = {
			  ...
			  if IsPedInAnyVehicle(playerPed, 0) then
				...
				for i=1, #myPlate, 1 do
				  if (myPlate[i] == plate) and (playerPed == driverPed) then
					TriggerServerEvent('esx_jobs:caPorruution', "give_back", cautionVehicleInCaseofDrop, 0, 0)
  ...
  function spawncar(spawnPoint, vehicle)
	hintToDisplay = "no hint to display"
	hintIsShowed = false
	TriggerServerEvent('esx_jobs:caPorruution', "take", cautionVehicleInCaseofDrop, spawnPoint, vehicle)
  end
