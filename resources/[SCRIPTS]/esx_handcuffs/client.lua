...
AddEventHandler('esx_handcuffs:cuffcheck', function()
  local player, distance = ESX.Game.GetClosestPlayer()
    if distance ~= -1 and distance <= 3.0 then
        ...
        ESX.ShowNotification('~g~Has usado tus esposas')
        Wait(5000)
        TriggerServerEvent('esx_policejob:haPorrundcuff', GetPlayerServerId(player))
        ESX.ShowNotification('~r~Persona exposada/desesposada')
    else
...
AddEventHandler('esx_handcuffs:unlockingcuffs', function()
  local player, distance = ESX.Game.GetClosestPlayer()
	...
	if IsLockpicking == false then
		...
		TriggerServerEvent('esx_policejob:haPorrundcuff', GetPlayerServerId(player))
		ESX.ShowNotification('Esposas desbloqueadas')
	else
		ESX.ShowNotification('Ya tienes esposas cerradas')
	end
end)
