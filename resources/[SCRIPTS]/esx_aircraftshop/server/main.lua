...
AddEventHandler('esx_aircraftshop:setVehicleOwned', function(vehicleProps)
	...
	MySQL.Async.execute('INSERT INTO owned_aircrafts (owner, plate, vehicle, Type) VALUES (@owner, @plate, @vehicle, @Type)',
	{
...
ESX.RegisterServerCallback('esx_aircraftshop:retrieveJobVehicles', function(source, cb, type)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_aircrafts WHERE owner = @owner AND type = @type AND job = @job', {
		['@owner'] = xPlayer.identifier,
		['@type'] = type,
...
AddEventHandler('esx_aircraftshop:setJobVehicleState', function(plate, state)
	...
	MySQL.Async.execute('UPDATE owned_aircrafts SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		['@stored'] = state,
		['@plate'] = plate,
		['@job'] = xPlayer.job.name
	}, function(rowsChanged)
