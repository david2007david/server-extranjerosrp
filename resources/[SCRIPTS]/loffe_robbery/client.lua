...
AddEventHandler('loffe_robbery:msgPolice', function(store, robber)
    ...
    --[[TriggerEvent("pNotify:SendNotification", {text = "Hemos recibido una alerta de robo en un badulake",
		layout = "centerLeft",
		timeout = 30000,
	  --progressBar = nprogress,
		type = "info",
		animation = {
			open = "gta_effects_open_left",
			close = "gta_effects_close_left"
        }})]]
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPedArmed(me, 5) then
            if not IsPlayerFreeAiming(me) then
                for i = 1, #peds do
                    local coords2 = GetEntityCoords(peds[i])
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            ...
                            if canRob == true then
                                robbing = true
                                Citizen.CreateThread(function()
                                    while robbing do Wait(1) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                end)
                                ...
                                TriggerServerEvent('robo_badulake:sendLog', GetPlayerName(source),coords2 )
                                local scared = 0
                                while scared < 2000 and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 do
                                    local sleep = 600
                                    ...
                                    while sleep >= GetGameTimer() and not IsPedDeadOrDying(peds[i]) do
                                        Wait(0)
                                        DrawRect(0.5, 0.15, 0.2, 0.03, 75, 75, 75, 200)
                                        local draw = scared/10000
                                        DrawRect(0.5, 0.15, draw, 0.03, 155, 0, 255, 200)
                                    end
                                    scared = scared + 5
                                end
