...
ESX.ShowNotification=function(a,b)
	if exports.mole_notifications~=nil then
	exports.mole_notifications:MoleShowNotification(a,b)else 
	AddTextEntry('esxNotification',a)
	SetNotificationTextEntry('esxNotification')
	DrawNotification(false,true)end end;
	
	
ESX.ShowAdvancedNotification=function(c,d,a,e,f,b)
	if exports.mole_notifications~=nil then 
	exports.mole_notifications:MoleShowAdvancedNotification(c,d,a,e,f,b)else AddTextEntry('esxAdvancedNotification',a)
	SetNotificationTextEntry('esxAdvancedNotification')
	SetNotificationMessage(e,e,false,f,c,d)DrawNotification(false,false)
	end end
...
ESX.ShowFloatingHelpNotification = function(msg, coords)
    ...
    SetFloatingHelpTextStyle(1, 1, 1, -1, 3, 0)
			...
			if data1.current.action == 'give' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
				{
					...
					if type == 'item_weapon' then
						...
						TriggerServerEvent('esx:giPorruveInventoryItem', GetPlayerServerId(nearbyPlayer), type, item, sourceAmmo)
						menu2.close()
						menu1.close()
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
							...
							if quantity ~= nil then
								TriggerServerEvent('esx:giPorruveInventoryItem', GetPlayerServerId(nearbyPlayer), type, item, quantity)
