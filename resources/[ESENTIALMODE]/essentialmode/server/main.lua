...
AddEventHandler('playerConnecting', function(name, setKickReason)
	...
	if not id then
		setKickReason("No se puede encontrar SteamID, reinicie FiveM con Steam abierto o reinicie FiveM y Steam si Steam ya está abierto")
		CancelEvent()
	end
end)
...
AddEventHandler("playerConnecting", function (name, kick, defer)
    ...
    if (name:find("<") ~= nil or name:find(">") ~= nil) then
        defer.done("Tu nombre tiene caracteres no permitido en este servidor")
        return
    end
