...
function TeleportToWaypoint()
	...
	if DoesBlipExist(WaypointHandle) then
		local wCoords = GetBlipInfoIdCoord(WaypointHandle)

		for height = 1, 1000 do
			SetPedCoordsKeepVehicle(PlayerPedId(), wCoords["x"], wCoords["y"], height + 0.0)

			local foundGround, zPos = GetGroundZFor_3dCoord(wCoords["x"], wCoords["y"], height + 0.0)
		...
		ESX.ShowNotification("No has marcado ningun punto")
	end
...
function GenerateMenu() -- this is a big ass function
	...
	nessx = _menuPool:AddSubMenu(mainMenu, GetLocalisedText("nessx"),"",true)
	...
	nessx:SetMenuWidthOffset(menuWidth)
	...
	if permissions["manageserver"] then
		ESX.ShowAdvancedNotification('~r~[Menu Admin]', 'Bienvenido/a al ~b~menu de staff', 'Modificado por ~b~RUBIO. \nCON MUCHO ❤\n~b~SolterosRP', 'CHAR_AMANDA', 8)

		local thisItem = NativeUI.CreateItem(GetLocalisedText("noclip"), GetLocalisedText("noclip_exp"))
		nessx:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			admin_no_clip()
		end

		local thisItem = NativeUI.CreateItem(GetLocalisedText("tpm"), GetLocalisedText("tpm_exp"))
		nessx:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TeleportToWaypoint()
		end

		local thisItem = NativeUI.CreateItem(GetLocalisedText("v_repair"), GetLocalisedText("v_repair_exp"))
		nessx:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			vehicle_repair()
		end
